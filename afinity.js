...
(function() {
  ...
  const shippingPageObserver = new MutationObserver((mutations, observer) => {
    ...
    const manageAddressesLink = document.querySelector('a[aria-label="Manage addresses"]');
  ...
  const overviewPageObserver = new MutationObserver((mutations, observer) => {
    ...
    const editAddressButton = document.querySelector('[data-testid="recharge-internal-edit-address-button"]');
  ...
  function handleOrdersPage() {
    ...
    const ordersSection = document.querySelector('.recharge-section-orders-previous');
    if (ordersSection) {
  ...
  function getFulfillmentDate(order) {
    ...
    const fulfillmentDateAttr = customAttrs.find(attr => 
      attr.key === 'Fulfillment Date'
    );
  ...
  async function loadCustomerOrders(hashedUserId) {
    try {
      const response = await fetch(`${API_URL}/subscriptions/customer/${hashedUserId}/orders`);
      if (!response.ok) {
  ...
  function renderOrdersSection() {
    ...
    const ordersHTML = currentOrders.map(order => {
      ...
      const totalPrice = parseFloat(order.totalPriceSet.presentmentMoney.amount);
      ...
      return `
        <div class="order-item" data-order-id="${order.id}">
          <div class="order-header">
            ...
            <button class="order-details-toggle" onclick="toggleOrderDetails('${order.id}')">
    ...
    const paginationHTML = generatePaginationControls(totalPages, currentOrdersPage);
  ...
  function renderOrderDetails(order) {
    ...
    const deliveryAddress = order.displayAddress?.formatted || 'No address provided';
    
    const lineItems = order.lineItems.edges.map(edge => edge.node);
    const itemsHTML = lineItems.map(item => {
      const itemPrice = parseFloat(item.originalUnitPriceSet.presentmentMoney.amount);
      ...
      const tableInfo = customAttrs.find(attr => attr.key === 'table')?.value || '';
      
      return `
        <div class="order-item-detail">
          <div class="item-image">
            ${imageUrl ? `<img src="${imageUrl}" alt="${item.name}" />` : '<div class="no-image">No image</div>'}
          </div>
          <div class="item-info">
            <div class="item-name">${item.name}</div>
            ${tableInfo ? `<div class="item-table">${tableInfo}</div>` : ''}
            <div class="item-quantity-price">${item.currentQuantity} x ${formattedItemPrice}</div>
          </div>
    ...
    return `
      <div class="order-details-content">
        ...
        <div class="order-details-items">
          <h4>In your ${new Date(orderDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric' })} order</h4>
          ${itemsHTML}
        </div>
  ...
  function generatePaginationControls(totalPages, currentPage) {
    ...
    // Previous button
    if (currentPage > 1) {
      paginationHTML += `<button class="pagination-btn prev" onclick="goToOrdersPage(${currentPage - 1})">Previous</button>`;
    } else {
    ...
    if (currentPage < totalPages) {
      paginationHTML += `<button class="pagination-btn next" onclick="goToOrdersPage(${currentPage + 1})">Next</button>`;
    } else {
  ...
  window.toggleOrderDetails = function(orderId) {
    ...
    const toggleButton = document.querySelector(`[data-order-id="${orderId}"] .order-details-toggle`);
  ...
  function renderPickupLocationsSection() {
    ...
    if (section) {
      ...
      const initialSearchBtn = section.querySelector('#afinity-search-save-address-btn');
      if (initialSearchBtn) {
      ...
      if (methodSelect) {
        ...
        methodSelect.addEventListener('change', async (e) => {
          ...
          const searchButton = document.getElementById('afinity-search-save-address-btn');
          if (searchButton) {
  ...
  function renderMethodSection() {
    ...
    if(method === 'Delivery') {
      ...
      fetchAvailableDates(zip, null).then(() => {
        setupDatePicker(method);
        // Re-initialize time picker for pickup
        reinitializeTimePicker();
        // Validate date and time selection after clearing (should be disabled)
        modalChanges.selectedPickupLocationId = window.deliveryLocation.location_id
  ...
  function getFulfillmentTimeFromSubscription() {
    ...
    const fulfillmentDateEntry = currentSubscription.include.address.order_attributes.find(attr => {
      if (attr && typeof attr === 'object') {
        if ('name' in attr && 'value' in attr) {
    ...
    const fulfillmentDateAttr = fulfillmentDateEntry ? ('value' in fulfillmentDateEntry ? fulfillmentDateEntry.value : fulfillmentDateEntry["Fulfillment Date"]) : null;
  ...
  async function handleConditionalFees(items, subscriptionId) {
    try {
      ...
      const subscriptionDeliveryFeeWaiverThresholdCents = Math.round(parseFloat(settings.find((s) => s.key === 'subscription_delivery_fee_waiver_threshold')?.value || '0') * 100);
      ...
      items.forEach(item => {
        ...
        const finalPriceCents = shouldApplyDiscount ? Math.floor(price * 0.9) : price;
      ...
      if (currentFulfillmentMethod === 'Delivery' && totalCents < subscriptionDeliveryFeeWaiverThresholdCents) {
        const deliveryFee = {
          collection_id: '308869562425',
                        ...
                        <div>
                          ...
                          <div class="afinity-modal-cart-details">
                            ...
                            <div class="afinity-modal-cart-price">
                              <span class="afinity-modal-cart-price--discount">$${discountedPrice.toFixed(2)}</span>
                              <span class="afinity-modal-cart-price--original">$${originalPrice.toFixed(2)}</span>
                            </div>
                          </div>
                        </div>
                      </div>
            ...
            <div class="afinity-r-card__details">
              ...
              <div class="afinity-r-card__footer">
                <div class="price__container price-block" data-variant-id="${variant.id}">
                  ...
                  <span class="afinity-r-card__price--default">$${price.toFixed(2)}</span>
                </div>
  ...
  function validateDateAndTimeSelection() {
    ...
    const currentMethod = modalChanges.fulfillmentMethod || fulfillmentMethod || 'Delivery';
    if (currentMethod === 'Delivery') {
      const zipCode = modalChanges.zip || zip;
      if (zipCode && zipCode.length === 5) {
        // Check if we have delivery days and location data
        const hasDeliveryDays = window.deliveryDaysData && Array.isArray(window.deliveryDaysData) && window.deliveryDaysData.length > 0;
        const hasDeliveryLocation = window.deliveryLocation && Object.keys(window.deliveryLocation).length > 0;
  ...
  async function saveDate() {
    ...
    if (currentMethod === 'Delivery') {
      ...
      if (zipCode && zipCode.length === 5) {
        ...
        if (!hasDeliveryDays || !hasDeliveryLocation) {
          showToast(`Zip code ${zipCode} is not deliverable. Please enter a different zip code.`, 'error');
          return;
        }
      } else {
    ...
    try {
      ...
      try {
        ...
        let dateToUse = modalChanges.deliveryDate || deliveryDate || getDeliveryDateFromSubscription();
    ...
    if (swapBtn) {
      ...
      if (mealsPageMode === 'update') {
        ...
        try {
          ...
          const finalItems = await handleConditionalFees(itemsWithoutFees, subscriptionId);
  ...
  async function performZipCodeAvailabilitySearch(zipCode) {
    try {
      ...
      const response = await fetch(`${API_URL}/search/availability/${encodeURIComponent(zipCode)}`);
      const data = await response.json();
      
      // Check if zip code is deliverable
      const hasDeliveryDays = data.deliveryDays && Array.isArray(data.deliveryDays) && data.deliveryDays.length > 0;
      const hasDeliveryLocation = data.deliveryLocation && Object.keys(data.deliveryLocation).length > 0;
      ...
      showToast('Error checking zip code availability. Please try again.', 'error');
  ...
  async function searchAndSaveAddress() {
    ...
    if (currentMethod !== 'Delivery') {
      showToast('Address validation is only available for Delivery method.', 'info');
      return;
    }
    ...
    try {
      ...
      showToast('Zip code is deliverable, please select the date and time!', 'success');
        ...
        swapBtn.onclick = async () => {
          if (mealsPageMode === 'update') {
            ...
            try {
              ...
              // Apply conditional fee logic with items excluding existing fees
              items = await handleConditionalFees(itemsWithoutFees, subscriptionId);
  ...
  function reinitializeTimePicker() {
    ...
    if (timeSelect) {
      ...
      let defaultTime24 = modalChanges.fulfillmentTime || subscriptionTime || fulfillmentTime;
  ...
  function refreshMealsDisplayForDateChange() {
    ...
    document.addEventListener('Recharge::click::manageSubscription', function(event) {
      ...
      const subscriptionId = event.detail?.payload?.subscriptionId || event.detail?.subscription_id;
