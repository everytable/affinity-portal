...
(function () {
  ...
  // Use ontouchend to invoke onclick ONCE on touch devices, then suppress the follow-up click
  (function enableTouchEndOnceForOnclick() {
    var TOUCH_FLAG = '__afinity_touch_invoke__';
    var TOUCH_TS = '__afinity_touch_ts__';

    function attachTouchEndOnce(el, handler) {
      if (!el || typeof handler !== 'function') return;
      el.ontouchend = function (e) {
        try {
          if (e && typeof e.preventDefault === 'function') e.preventDefault();
        } catch (_) {}
        if (el[TOUCH_FLAG]) return;
        el[TOUCH_FLAG] = true;
        el[TOUCH_TS] = Date.now();
        try {
          handler.call(el, e);
        } finally {
          setTimeout(function () {
    ...
    function initElement(el) {
      ...
      var handler = typeof el.onclick === 'function' ? el.onclick : null;
      if (handler) attachTouchEndOnce(el, handler);
    }

    function initWithin(root) {
      if (!root) return;
      if (root instanceof Element) initElement(root);
      var all = root.querySelectorAll ? root.querySelectorAll('*') : [];
      for (var i = 0; i < all.length; i++) initElement(all[i]);
    }
    ...
    var observer = new MutationObserver(function (mutations) {
      for (var idx = 0; idx < mutations.length; idx++) {
        ...
        if (m.type === 'childList') {
          if (m.addedNodes) {
            for (var j = 0; j < m.addedNodes.length; j++) initWithin(m.addedNodes[j]);
          }
        } else if (m.type === 'attributes' && m.attributeName === 'onclick') {
          initElement(m.target);
        }
      }
    });
    try {
      ...
      if (desc && typeof desc.set === 'function' && typeof desc.get === 'function') {
        Object.defineProperty(HTMLElement.prototype, 'onclick', {
          ...
          set: function (handler) {
            ...
            try {
              attachTouchEndOnce(this, handler);
            } catch (_) {}
          }
        });
      }
    ...
    // As a safety net, suppress the immediate native click after a handled touchend
    document.addEventListener(
      'click',
      function (e) {
        var target = e.target instanceof Element ? e.target.closest('[onclick]') : null;
        if (!target) return;
        if (target[TOUCH_TS] && Date.now() - target[TOUCH_TS] < 500) {
