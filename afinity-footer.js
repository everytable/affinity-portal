(function () {
  ...
  const datePattern = /\b(?:Sun|Mon|Tue|Wed|Thu|Fri|Sat),?\s+(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},\s+\d{4}\b/;
  ...
  function hideEditButtonNearby(el) {
    const editBtn = el.parentElement?.querySelector('[data-testid="edit-button"], button[aria-label="Edit"]');
    if (editBtn) {
  ...
  function updateRechargeHeadings() {
    document.querySelectorAll(combinedSelector).forEach((el) => {
      ...
      console.log(`✅ Updated Recharge heading: "${originalDate}" → "${newDate}"`);
    });
  }
  ...
  const observer = new MutationObserver((mutations) => {
    for (const mutation of mutations) {
      ...
      const matching = nodes.some((node) =>
        node.nodeType === 1 &&
        (
  ...
  /// Show only 4 items in the div
    function limitUpcomingOrdersToFour() {
        ...
        const upcomingOrdersDiv = Array.from(document.querySelectorAll('div'))
